...
class Element {
  constructor() {
    ...
    _defineProperty(this, "toValue", (value) => value * this.dpr);
...
class Wave extends Element {
  constructor(pointAmount, _p, _p2) {
    super();
    _defineProperty(
      this,
      "draw",
    ...
    this.vertices = new Array(pointAmount)
      .fill(null)
      .map((p, i) => ({ x: _p.x + vx * i, y: _p.y + vy * i }));
  }
}
...
class Gate extends Element {
  ...
  setupWaves() {
    ...
    for (let i = -extend; i < this.amount + extend; i++) {
      ...
      const p1 = { x: this.px, y: this.py + dy };
      const p2 = { x: this.px + this.pw, y: this.py + dy };
...
class Ring extends Element {
  constructor({
    ...
    this.center = { x: this.hw, y: this.hh };
    for (let i = 0; i < pointAmount; i++) {
      ...
      this.points.push({ x, y, radian });
    }
  }
  get stroke() {
    this.opacityStroke -= this.opacityDecay;
    const r = Math.floor(170 + Math.sin(this.decay / 10) * 60);
    const g = Math.floor(130 + Math.sin(this.decay / 5) * 40); // const b = Math.floor(130 + (Math.sin(this.decay / 5) * 40));
...
class Background extends Element {
  constructor(...args) {
    super(...args);
    _defineProperty(this, "draw", ({ ctx, canvas }) => {
...
class Canvas {
  constructor(elements = []) {
    ...
    this.ctx.scale(this.dpr, this.dpr); // stores
    this.elements = elements;
    this.tick = 0;
    this.mouse = { x: 0, y: 0 }; // run
    this.setCanvasSize();
    this.setupListeners();
  }
  setupListeners() {
